...
namespace SCP682Exiled
{
    public class EventHandler
    {
        ...
        public void OnSetRole(ChangingRoleEventArgs ev)
        {
            if (ev.NewRole == RoleType.Scp93989 && !scp682.Contains(ev.Player.UserId))
            {
                ...
                if (s <= SCP682.Singleton.Config.spawn_chance)
                {
                    ...
                    ev.Player.MaxArtificialHealth = SCP682.Singleton.Config.MaxAHP;
                    ev.Player.ArtificialHealth = SCP682.Singleton.Config.MaxAHP;
                    ...
                    ev.Player.CustomInfo = $"{SCP682.Singleton.Config.DisplayName}";
        ...
        public IEnumerator<float> RegenerateAHPSCP682(Player ply)
        { 
            while (true)
            {
                if (ply.Role == RoleType.Scp93989 && scp682.Contains(ply.UserId) && (ply.ArtificialHealth < SCP682.Singleton.Config.MaxAHP))
                { 
                    ply.ArtificialHealth = ply.ArtificialHealth + SCP682.Singleton.Config.RegAHP;
                }
                 yield return Timing.WaitForSeconds(SCP682.Singleton.Config.ahp_time);
            }
        }
